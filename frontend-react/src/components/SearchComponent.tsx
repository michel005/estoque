...
export default function SearchComponent() {
    ...
    function eventoEnter(buscaAtu = buscaAtual) {
        ...
        if (opcoes.fim === true) {
            ...
            zerarBusca();
        } else if (opcoes[buscaGeral.value].fim === true) {
        ...
        <SearchStyle className={resultados.length === 0 && buscaAtual.length === 0 ? "" : "mostrar"}>
            ...
            <div className="caixaBuscaContainer">
                ...
                {resultados.map((value: any, index: any) => {
                    if (value.fim === false) {
                        return (
                            <div key={index} className="resultado" onClick={() => proximoComando(value)}>
                                <div className="icone">
                                    <FontAwesomeIcon icon={solid("bahai")} /> {value.nomeComando}
                                </div>{" "}
                                <div className="descricao">{value.nome}</div>
                            </div>
                        );
                    } else {
                        return (
                            <div key={index} className="resultado comando" onClick={() => executaComando(value)}>
                                <div className="icone">
                                    <FontAwesomeIcon icon={solid("play")} /> {value.nomeComando}
                                </div>{" "}
                                <div className="descricao">{value.nome}</div>
                            </div>
                        );
                ...
                <div className="resultado" onClick={zerarBusca}>
                    <div className="icone">
                        <FontAwesomeIcon icon={solid("play")} /> Fechar busca
                    </div>
                </div>
            </div>
        </SearchStyle>
