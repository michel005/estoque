...
public interface ItemRepository extends JpaRepository<Item, Long> {
    ...
    @Query("select new com.michel.estoque.model.QuantidadeItemModel( x, coalesce(sum(i.quantidade), 0), coalesce(min(i.valor / i.quantidade), 0), coalesce(max(i.valor / i.quantidade), 0) ) from ItemEventoEntrada i right join i.item x where i.item.nome like '%' || :nome || '%' and ( :categoria = '' or i.item.categoria = :categoria ) group by x")
